...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const adminEmail = process.env.SITE_ADMIN_EMAIL || 'praesultv@gmail.com'
    if (!session?.user?.email || session.user.email !== adminEmail) return res.status(403).json({ ok: false, message: 'Forbidden' })
